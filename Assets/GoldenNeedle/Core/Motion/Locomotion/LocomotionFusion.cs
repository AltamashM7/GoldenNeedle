...
namespace GoldenNeedle.Core.Motion.Locomotion
{
    ...
    {
        [Tooltip("Game-world scale applied to authoritative physical left/right displacement.")]
        [Min(0f)] public float lateralScale = 0.9f;

        [Tooltip("Game-world scale applied to authoritative physical toward/away displacement.")]
        [Min(0f)] public float depthScale = 1.5f;

        [Tooltip("Camera-space lateral displacement ignored around the physical origin before scaling.")]
        [Min(0f)] public float lateralDeadzone = 0.012f;

        [Tooltip("Camera-space depth displacement ignored around the physical origin before scaling.")]
        ...
        [Tooltip("Minimum trusted root confidence required for a live physical update.")]
    ...
    /// <summary>
    /// Maps the already-authoritative physical displacement into world X/Z and blends cadence.
    /// It intentionally owns no second stationary/commit/rebase state machine. Temporary root loss
    /// only holds the last mapped contribution; all physical continuity belongs to the root tracker.
    /// </summary>
    public sealed class LocomotionFusion
    {
            ...
            CanonicalToAvatarAxisMap physicalReferenceMap)
        {
            ...
            if (rootUsable)
            {
                var scaledCameraDisplacement = new Vector3(
                    ApplyDeadzone(root.displacementXZ.x, _settings.lateralDeadzone) *
                        _settings.lateralScale,
                    0f,
                    ApplyDeadzone(root.displacementXZ.y, _settings.depthDeadzone) *
                        _settings.depthScale);
            ...
            return new LocomotionFusionResult
            {
                ...
                cadenceActive = cadence.active && cadenceBlend > 0.05f &&
        ...
        {
            if (!referenceMap.IsValid || !IsFinite(scaledCameraVector))
            {
                return Vector2.zero;
            }
