...
| Single canonical camera / inference frame | **CURRENT PHASE 4 FOUNDATION** | Sensor/storage and pixel preparation construct the correctly oriented image presented to MediaPipe. Canonical normalized/world mapping uses only the semantic `x, 1-y` and `x, -y, z` conversions; input H/V/rotation are not mechanically applied to returned world data, and display mirroring remains presentation-only. |
