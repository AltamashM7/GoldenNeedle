...
| Separate camera inference and display transforms | **CURRENT PHASE 2 IMPLEMENTATION** | Unity-to-MediaPipe texture preparation, physical sensor metadata, display mirroring, and overlay conversion are separate transforms so the inference-only vertical flip cannot invert the user-facing preview. |
